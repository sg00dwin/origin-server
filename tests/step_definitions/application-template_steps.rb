...
\n    Required: false\n    Type: FILESYSTEM:doc-root\nVendor: unknown\nVersion: \"0.0\"\n"
  ...
  output = `oo-admin-ctl-template --named 'Ruby on Rails' --metadata '/tmp/metadata.json' --git-url 'https://github.com/openshift/rails-example' --command 'add' --cost '1' --descriptor '/tmp/descriptor.yaml' --tags 'ruby,rails,framework'`
  @template_uuid = output.split(" ")[1]
end

Then /^the application created from the template should be accessible$/ do
  retry_times = 0
  begin
...
When /^I remove the template$/ do
  output = `oo-admin-ctl-template -c remove -u "#{@template_uuid}"`
end
