...
Feature: Application template list
  ...
  Scenario: Add a template and create an application
    ...
    When I send a POST request to "/domains" with the following:"id=api"
    Then the response should be "201"
    When I create a new application named 'fooapp' within domain 'api' with the template
    Then the response should be "201"
    When I send a GET request to "/domains/api/applications/fooapp"
    Then the response should be "200"
    When I send a DELETE request to "/domains/api/applications/fooapp"
    Then the response should be "204"
    When I send a DELETE request to "/domains/api"
    Then the response should be "204"
