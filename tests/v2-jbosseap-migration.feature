...
Feature: V2 Migrations for V1 apps
 Scenario: jbosseap-6.0 app migration
  ...
  Scenario: Stopped jbosseap-6.0 app migration
    Given a new client created jbosseap-6.0 application
  ...
  Scenario: jbosseap-6.0 + mysql-5.1 migration
    Given a new client created jbosseap-6.0 application
  ...
  Scenario: Scaled jbosseap-6.0 + mysql-5.1 migration
    Given a new client created scalable jbosseap-6.0 application
    Given the embedded mysql-5.1 cartridge is added
    Then I can select from mysql
