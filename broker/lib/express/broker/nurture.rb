  ...
  module Broker
    class Nurture
      ...
      def self.application(rhlogin, user_uuid, app_name, user_namespace, type, action, app_uuid)
          ...
          `curl -s -O /dev/null -X POST -u '#{Rails.application.config.analytics[:nurture_username]}:#{Rails.application.config.analytics[:nurture_password]}' '#{Rails.application.config.analytics[:nurture_url]}applications' \
