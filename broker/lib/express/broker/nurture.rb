  ...
  module Broker
    class Nurture
      ...
      def self.application(rhlogin, user_uuid, app_name, user_namespace, type, action, app_uuid)
          return unless (Rails.application.config.cdk[:nurture_enabled].to_s.downcase == 'true')
          ...
          `curl -s -O /dev/null -X POST -u '#{Rails.application.config.cdk[:nurture_username]}:#{Rails.application.config.cdk[:nurture_password]}' '#{Rails.application.config.cdk[:nurture_url]}applications' \
      ...
      def self.libra_contact(rhlogin, uuid, user_namespace, action)
          ...
          `curl -s -O /dev/null -X POST -u '#{Rails.application.config.cdk[:nurture_username]}:#{Rails.application.config.cdk[:nurture_password]}' '#{Rails.application.config.cdk[:nurture_url]}/libra_contact' \
