  ...
  module Broker
    class Apptegic
      ...
      def self.application(rhlogin, user_uuid, app_name, user_namespace, type, action, app_uuid)
          return unless (Rails.application.config.libra[:apptegic_enabled].to_s.downcase == 'true')
          ...
          `curl -s -O /dev/null '#{Rails.application.config.libra[:apptegic_url]}' \
      --data-urlencode '_ak=#{Rails.application.config.libra[:apptegic_key]}' \
      --data-urlencode '_at=#{Rails.application.config.libra[:apptegic_secret]}' \
      --data-urlencode '_ds=#{Rails.application.config.libra[:apptegic_dataset]}' \
