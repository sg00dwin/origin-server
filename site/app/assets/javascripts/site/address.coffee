...
$ ->
  ...
  add_button = $("<a class='icon-plus add-address' aria-hidden='true' href='#' onclick='return false;' title='Add Address Line' />")
  $('.address-line:not(:last)').parent().append(add_button)
  # Hide empty address lines
  $('.address-line:not(:first):text[value=""]').parents('.control-group').hide()
  # Hide unused buttons
  $('a.add-address:visible').not(':last').hide()

  # Show the next address line and remove button
  $('a.add-address').on 'click', ->
    $(this).parents('.control-group').next(':has(.address-line)').show()
    $(this).detach()
  ###
