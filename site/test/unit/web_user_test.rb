...
class WebUserTest < ActiveSupport::TestCase
  ...
  test 'email from prohibited domain is not valid' do
    ...
    assert !user.valid?
    assert user.errors.values.flatten.include? 'OpenShift Online does not allow creating accounts with email addresses from anonymous mail services due to security concerns. Please use a different email address.'
  end
