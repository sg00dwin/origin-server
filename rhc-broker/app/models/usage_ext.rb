...
class Usage
  ...
  def get_usage_rate(plan_id)
    ...
    user_plan = plan_id.nil? ? Rails.configuration.billing[:default_plan] : plan_id.to_sym
    if defined?(Rails) && Rails.configuration.billing
      usage_rates = Rails.configuration.billing[:plans][user_plan][:usage_rates] rescue nil
    end
