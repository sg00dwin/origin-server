class PlansController < BaseController
  ...
  def index
    ...
    Express::AriaBilling::Plan.instance.plans.each do |key, value|
      plan = RestPlan.new(key, value[:name], value[:plan_no], value[:capabilities], value[:charges])
      plans.push(plan)
    end
  ...
  def show
    ...
    Express::AriaBilling::Plan.instance.plans.each do |key, value|
      return render_success(:ok, "plan",
                            RestPlan.new(key, value[:name], value[:plan_no], value[:capabilities], value[:charges]),
