...
log_message ":START: $freq cron run for stickshift user '$OPENSHIFT_APP_UUID'"
...
if [ -d "$SCRIPTS_DIR" ]; then
   ...
   separator=$(seq -s_ 75 | tr -d '[:digit:]')
   {
      ...
      if [ 124 -eq $status ]; then
         ...
         log_message "$wmsg [$MAX_RUN_TIME] for stickshift user '$OPENSHIFT_APP_UUID'" > /dev/null 2>&1
         echo "$wmsg"
      fi
...
log_message ":END: $freq cron run for stickshift user '$OPENSHIFT_APP_UUID'"
exit 0
