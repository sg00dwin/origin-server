  ...
  module Agent
    ...
    class Libra<RPC::Agent
      ...
      def cartridge_do_action
        ...
        validate :action, /\A(configure|deconfigure|preconfigure|update_namespace|add-env-var|remove-env-var|remove-authorized-ssh-key|add-authorized-ssh-key|remove-authorized-ssh-key|add-broker-auth-key|remove-broker-auth-key|tidy|deploy_httpd_proxy|remove_httpd_proxy|move|info|post-install|post_remove|pre-install|reload|restart|start|status|stop|force-stop|add-alias|remove-alias)\Z/
