  ...
  module Agent
    ...
    class Libra<RPC::Agent
      ...
      def cartridge_do_action
        ...
        validate :action, /\A(configure|deconfigure|preconfigure|update_namespace|add-authorized-ssh-key|remove-authorized-ssh-key|add-broker-auth-key|remove-broker-auth-key|info|post-install|post_remove|pre-install|reload|restart|start|status|stop)\Z/
