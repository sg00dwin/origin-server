...
action "cartridge_do", :description => "run a cartridge action" do
    ...
    input :action,
        ...
        :type           => :string,
        :validation     => '^(configure|deconfigure|preconfigure|update_namespace|add-env-var|remove-env-var|add-authorized-ssh-key|remove-authorized-ssh-key|add-broker-auth-key|remove-broker-auth-key|deploy_httpd_proxy|info|post-install|post_remove|pre-install|reload|restart|start|status|stop)$',
        :optional       => false,
        :maxlength      => 64
