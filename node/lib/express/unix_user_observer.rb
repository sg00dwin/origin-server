...
module StickShift
  class UnixUserObserver
    ...
    def after_initialize_homedir(user)
      cmd = "/bin/sh #{File.join('/usr/libexec/stickshfit/lib', "express/setup_pam_fs_limits.sh")} #{user.name} #{user.quota_blocks ? user.quota_blocks : ''} #{user.quota_files ? user.quota_files : ''}"
    ...
    def before_unix_user_destroy(user)
      ...
      cmd = "/bin/sh #{File.join("/usr/libexec/stickshfit/lib", "express/teardown_pam_fs_limits.sh")} #{user.name}"
      shellCmd(cmd)
    end
