...
module Cloud
  module Sdk
    module Broker
      class ApplicationContainerProxy < Cloud::Sdk::ApplicationContainerProxy
        attr_accessor :id
        ...
        def get_available_cartridges
          reply = exec_command('cloud-sdk', 'cartridge-list', '--porcelain --with-descriptors')
        ...
        def create(app, gear)
          ...
          reply = exec_command("cloud-sdk","app-create", args)
          parse_result(reply)
        end
      
        def destroy(app, gear)
          args = "--with-app-uuid '#{app.uuid}' --with-container-uuid '#{gear.uuid}'"
          reply = exec_command("cloud-sdk","app-destroy", args)
          parse_result(reply)
        end
        ...
        def execute_connector(app, gear, cart, connector_name, input_args)
          ...
          reply = exec_command("cloud-sdk","connector-execute", args)
        ...
        def has_app?(app_uuid, app_name)
          if File.exist?("/var/lib/cloud-sdk/#{app_uuid}/#{app_name}")
            return true
          end
          return false
        end
        ...
        def has_embedded_app?(app_uuid, embedded_type)
          if File.exist?("/var/lib/cloud-sdk/#{app_uuid}/#{embedded_type}")
            return true
          end
          return false
        end
