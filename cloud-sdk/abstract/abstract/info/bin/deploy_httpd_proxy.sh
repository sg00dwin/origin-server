...
rm -rf "/etc/httpd/conf.d/cloud-sdk/${uuid}_${namespace}_${application}.conf" "/etc/httpd/conf.d/cloud-sdk/${uuid}_${namespace}_${application}"

mkdir "/etc/httpd/conf.d/cloud-sdk/${uuid}_${namespace}_${application}"

cat <<EOF > "/etc/httpd/conf.d/cloud-sdk/${uuid}_${namespace}_${application}/00000_default.conf"
  ServerName ${application}-${namespace}.${libra_domain}
  ServerAdmin mmcgrath@redhat.com
  DocumentRoot /var/www/html
  DefaultType None
EOF
cat <<EOF > "/etc/httpd/conf.d/cloud-sdk/${uuid}_${namespace}_${application}.conf"
<VirtualHost *:80>
  RequestHeader append X-Forwarded-Proto "http"

  Include /etc/httpd/conf.d/cloud-sdk/${uuid}_${namespace}_${application}/*.conf
