...
module Cloud::Sdk
  class BindDnsService < DnsService
    @cdk_dns_provider = Cloud::Sdk::BindDnsService

    # DEPENDENCIES
    # Rails.application.config.cdk[:domain_suffix]
    # Rails.application.config.cdk[:dns][:bind]
    ...
    def initialize(access_info = nil)
      if access_info != nil
        ...
        rails_config = Rails.application.config.cdk[:dns][:bind]
        ...
        @domain_suffix = Rails.application.config.cdk[:domain_suffix]
      else
