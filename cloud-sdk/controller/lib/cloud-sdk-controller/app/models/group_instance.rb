class GroupInstance < Cloud::Sdk::UserModel
  ...
  def elaborate(group, parent_comp_path, app)
    ...
    group.component_refs.each { |comp_ref_name, comp_ref|
      cpath = parent_comp_path + "." + self.cart_name + ":" + self.profile_name + "." + comp_ref_name
      ci = ComponentInstance.new(self.cart_name, self.profile_name, comp_ref_name, cpath, self)
    ...
    group.auto_component_refs.each { |comp_ref_name, comp_ref|
      cpath = parent_comp_path + "." + self.cart_name + ":" + self.profile_name + "." + comp_ref_name
      ci = ComponentInstance.new(self.cart_name, self.profile_name, comp_ref_name, cpath, self)
      self.component_instances << cpath
      app.comp_instance_map[cpath] = ci
